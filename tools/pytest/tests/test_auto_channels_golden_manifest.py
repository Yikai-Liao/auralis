...
MANIFEST_PATH = REPO_ROOT / "tests" / "golden" / "auto_channels.toml"
SAMPLE_RATE = 48_000
PCM16_SCALE = np.float64(32_768.0)


def _load_auto_channel_cases() -> tuple[tuple[str, dict[str, Any]], ...]:
    manifest = tomllib.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    return tuple(sorted(manifest["id"].items()))


AUTO_CHANNEL_CASES = _load_auto_channel_cases()
...
@pytest.mark.parametrize(
    ("case_id", "case"),
    AUTO_CHANNEL_CASES,
    ids=[case_id for case_id, _ in AUTO_CHANNEL_CASES],
)
def test_cli_auto_channels_matches_sox_ng_golden_manifest(
    case_id: str,
    case: dict[str, Any],
    tmp_path: Path,
) -> None:
    ...
    assert metadata["auralis"]["channel_count"] == output_channels
    ...
    if failures:
        ...
        report["output_channels"] = output_channels
        report["sox_ng_auto_channels"] = bool(case["sox_ng_auto_channels"])
