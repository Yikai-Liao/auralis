...
MANIFEST_PATH = REPO_ROOT / "tests" / "golden" / "auto_dither.toml"
PCM16_SCALE = np.float64(32_768.0)


def _load_auto_dither_cases() -> tuple[tuple[str, dict[str, Any]], ...]:
    manifest = tomllib.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    return tuple(sorted(manifest["id"].items()))


AUTO_DITHER_CASES = _load_auto_dither_cases()
...
@pytest.mark.parametrize(
    ("case_id", "case"),
    AUTO_DITHER_CASES,
    ids=[case_id for case_id, _ in AUTO_DITHER_CASES],
)
def test_cli_auto_dither_matches_sox_ng_golden_manifest(
    case_id: str,
    case: dict[str, Any],
    tmp_path: Path,
) -> None:
    ...
    try:
        sox_result = run_sox_ng(
            ...
            sox_output,
            case["sox_ng"],
            disable_auto_dither=not bool(case["sox_ng_auto_dither"]),
        )
    ...
    if failures:
        ...
        report["sox_ng_auto_dither"] = bool(case["sox_ng_auto_dither"])
