...
#[test]
fn plan_timing_and_filter_recipes_use_graph_plan_contract() {
    let cases: [(&str, &[&str]); 11] = [
        ...
        (
            "pitch",
            &["120", "--quick", "--segment", "40", "--search", "20"],
        ),
        ("bass", &["-3", "--frequency", "120", "--width", "0.7q"]),
        ("treble", &["2", "--frequency", "4000", "--width", "1o"]),
        (
            "equalizer",
            &["--frequency", "1000", "--width", "500h", "--gain", "-2"],
        ),
        ("allpass", &["--frequency", "800", "--poles", "2"]),
        (
            "band",
            &["--frequency", "440", "--width", "100h", "--unpitched"],
        ),
        (
            "bandpass",
            &["--frequency", "1000", "--width", "0.7q", "--constant-skirt"],
        ),
        ("bandreject", &["--frequency", "1000", "--width", "0.7q"]),
        (
            "highpass",
            &["--frequency", "120", "--width", "0.7q", "--poles", "2"],
        ),
        ("lowpass", &["--frequency", "4000", "--width", "0.7q"]),
    ];
...
#[test]
fn plan_edit_recipes_use_graph_plan_contract() {
    let cases: [(&str, &[&str]); 6] = [
        ("fade", &["--in", "3", "--out", "2", "--curve", "linear"]),
        ("delay", &["--position", "1s"]),
        ("pad", &["--start", "2", "--at", "3@4", "--end", "5"]),
        ("repeat", &["2"]),
        ("downsample", &["3"]),
        ("upsample", &["2"]),
    ];
...
#[test]
fn plan_fir_dither_space_recipes_use_graph_plan_contract() {
    let cases: [(&str, &[&str]); 5] = [
        ("hilbert", &["--taps", "101"]),
        (
            "loudness",
            &["--gain", "-8", "--reference", "70", "--half-points", "511"],
        ),
        ("dither", &["--noise-shape", "shibata", "--precision", "20"]),
        (
            "reverb",
            &[
                "--wet-only",
        ...
        (
            "stretch",
            &[
                "1.2", "--window", "25", "--fade", "sqrt", "--shift", "0.4", "--fading", "0.5",
            ],
        ),
    ];
