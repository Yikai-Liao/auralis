...
fn chain_golden_manifest_renders_recorded_commands() {
    ...
    let boundary = manifest.get("chain_boundary_mix_gain_noop").unwrap();
    ...
    assert_eq!(boundary.combine_method(), Some("mix"));
    assert_eq!(
        boundary.render_auralis_command_line_with_inputs(
            "auralis",
            ["front.wav", "tail.wav"],
            "out.wav",
        ),
    ...
    assert_eq!(
        ...
        "sox_ng -R -D --combine mix front.wav tail.wav out.wav gain -3 : gain 0 : dcshift 0"
    );
