...
pub(super) fn parse_echo(effect: &'static str, args: &[&str]) -> CommandResult<EffectCommand> {
    ...
    for pair in tap_args.chunks_exact(2) {
        ...
        taps.push(EchoTap::new(delay_ms, decay).map_err(|source| {
    ...
    Ok(EffectCommand::Echo(
        Echo::new(gain_in, gain_out, taps).map_err(|source| {
            EffectCommandParseError::InvalidEffectConfig {
                effect,
                argument: "echo",
                source,
...
pub(super) fn render_echo(echo: &Echo) -> Vec<String> {
    let mut tokens = vec![
        "echo".to_owned(),
        render_f64(echo.gain_in()),
        render_f64(echo.gain_out()),
    ];
    for tap in echo.taps() {
...
mod tests {
    ...
    use crate::{Echo, EchoTap, EffectCommand, EffectCommandParseError};

    #[test]
    fn parses_one_or_more_delay_decay_pairs() {
        assert_eq!(
            parse_echo("echo", &["0.8", "0.9", "1", "0.5", "2.5", "-0.25"]).unwrap(),
            EffectCommand::Echo(
                Echo::new(
                    0.8,
                    0.9,
    ...
    #[test]
    fn render_echo_uses_canonical_numeric_tokens() {
        assert_eq!(
            parse_echo("echo", &["1.0", "0.5", "0.0", "0.25"])
                .unwrap()
                .render_tokens(),
            ["echo", "1", "0.5", "0", "0.25"]
        );
    ...
    fn requires_gain_and_complete_delay_decay_pairs() {
        assert!(matches!(
            parse_echo("echo", &["1", "1", "10"]).unwrap_err(),
        ...
        assert!(matches!(
            parse_echo("echo", &["1", "1"]).unwrap_err(),
            EffectCommandParseError::MissingArgument {
                argument: "delay-decay-pair",
                ..
            }
