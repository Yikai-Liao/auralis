...
proptest! {
    ...
    fn identity_parameters_preserve_samples(audio in generated_audio()) {
        ...
        let upsampled = Upsample::new(1)
            .expect("factor one is valid")
            .process_buffer(&source)
            .expect("factor one upsample cannot fail");
        prop_assert_sample_bits_eq(upsampled.as_planar_f32(), source.as_planar_f32())?;
        prop_assert_eq!(upsampled.frames(), source.frames());
        prop_assert_eq!(upsampled.channels(), source.channels());

        let same_speed = Speed::new(1.0)
            .expect("factor one is valid")
            .process_buffer(&source)
            .expect("factor one speed cannot fail");
        ...
        let same_stretch = Stretch::default()
            .process_buffer(&source)
            .expect("factor one stretch cannot fail");
    ...
    fn implemented_effects_preserve_finiteness_for_bounded_finite_input(
        ...
        let mut multiband_companded = source.clone();
        MCompand::parse_sox_args(&["0.01,0.1 -60,-60,0,-6"])
            .expect("fixture mcompand command is valid")
            .process_buffer(&mut multiband_companded)
            .expect("single-band mcompand reuses shared compand envelope");
        prop_assert_all_finite(&multiband_companded)?;
        ...
        let upsampled = Upsample::new(2)
            .expect("factor two is valid")
            .process_buffer(&source)
            .expect("fixture sample rate can be doubled");
        prop_assert_all_finite(&upsampled)?;

        let sped_up = Speed::new(1.5)
            .expect("fixture speed factor is valid")
            .process_buffer(&source)
            .expect("fixture sample rate can be multiplied");
        prop_assert_all_finite(&sped_up)?;

        let stretched = Stretch::new(1.5, 1.0)
            .expect("fixture stretch settings are valid")
            .process_buffer(&source)
        ...
        let cascaded_echoed =
            Echos::new(0.5, 0.5, [EchosTap::new(1.0, 0.25).expect("tap is valid")])
                .expect("echos fixture is valid")
                .process_buffer(&source)
