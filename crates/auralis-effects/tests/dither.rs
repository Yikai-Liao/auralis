...
use auralis_effects::{
    Dither, DitherMode, DitherState, EffectChainParseError, EffectCommand, EffectCommandParseError,
...
#[test]
fn dither_rejects_noise_shaping_auto_detect_and_bad_precision() {
    let shaping = parse_effect_chain(&["dither", "-s"]).unwrap_err();
    assert!(matches!(
        shaping,
