...
use auralis_effects::{
    parse_effect_command, Band, BandPass, BandReject, Bass, Biquad, BiquadCoefficients,
    BiquadWidth, Centercut, Channels, Contrast, DcShift, Deemph, EffectCommand, Equalizer, Fade,
    FadeCurve, Gain, HighPass, Loudness, LowPass, Oops, Pad, PositionedPad, Reverse, Saturation,
    SaturationType, SoftVol, Swap, Treble, Tremolo, Trim,
};
...
fn parses_supported_effect_commands_into_typed_configs() {
    let expected = [
        ...
        (&["deemph"][..], EffectCommand::Deemph(Deemph::new())),
        (
            &["trim", "1", "2"][..],
...
#[test]
fn parses_loudness_command_into_typed_config() {
    assert_eq!(
        parse_effect_command(&["loudness", "-6", "70", "127"]).unwrap(),
...
#[test]
fn parses_highpass_command_into_typed_config() {
    assert_eq!(
        parse_effect_command(&["highpass", "-1", "500"]).unwrap(),
        EffectCommand::HighPass(HighPass::one_pole(500.0).unwrap())
    );
}
