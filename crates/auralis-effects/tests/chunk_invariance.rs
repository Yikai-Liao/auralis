...
#[test]
fn treble_matches_whole_buffer_for_l5_chunk_matrix_when_state_is_preserved() {
    let treble =
        Treble::with_width(-6.0, 3000.0, BiquadWidth::slope(0.5)).expect("fixture design is valid");
    let source = stereo_source(1_105);
    let schedules = l5_chunk_schedules(frames_len(&source));
    ...
    for schedule in &schedules {
        ...
        treble
            .process_buffer(&mut whole)
            .expect("fixture sample rate keeps frequency below Nyquist");
        process_treble_by_channel_chunks(&mut chunked, treble, schedule);
...
#[test]
fn equalizer_matches_whole_buffer_for_l5_chunk_matrix_when_state_is_preserved() {
    let equalizer =
        Equalizer::new(1_000.0, BiquadWidth::q(1.0), 6.0).expect("fixture design is valid");
    let source = stereo_source(1_105);
    let schedules = l5_chunk_schedules(frames_len(&source));
    ...
    for schedule in &schedules {
        ...
        equalizer
            .process_buffer(&mut whole)
            .expect("fixture sample rate keeps frequency below Nyquist");
        process_equalizer_by_channel_chunks(&mut chunked, equalizer, schedule);
...
#[test]
fn lowpass_matches_whole_buffer_for_l5_chunk_matrix_when_state_is_preserved() {
    let low_pass =
        LowPass::with_width(1_000.0, BiquadWidth::q(0.707)).expect("fixture design is valid");
    let source = stereo_source(1_105);
    let schedules = l5_chunk_schedules(frames_len(&source));
    ...
    for schedule in &schedules {
        ...
        low_pass
            .process_buffer(&mut whole)
            .expect("fixture sample rate keeps frequency below Nyquist");
        process_lowpass_by_channel_chunks(&mut chunked, low_pass, schedule);
...
#[test]
fn highpass_matches_whole_buffer_for_l5_chunk_matrix_when_state_is_preserved() {
    let high_pass =
        HighPass::with_width(1_000.0, BiquadWidth::q(0.707)).expect("fixture design is valid");
    let source = stereo_source(1_105);
    let schedules = l5_chunk_schedules(frames_len(&source));
    ...
    for schedule in &schedules {
        ...
        high_pass
            .process_buffer(&mut whole)
            .expect("fixture sample rate keeps frequency below Nyquist");
        process_highpass_by_channel_chunks(&mut chunked, high_pass, schedule);
...
fn process_streaming_safe_chain_by_chunks(
    ...
    for command in chain.commands() {
        match command {
            ...
            EffectCommand::Equalizer(equalizer) => {
                process_equalizer_by_channel_chunks(audio, *equalizer, schedule);
            }
            EffectCommand::HighPass(high_pass) => {
                process_highpass_by_channel_chunks(audio, *high_pass, schedule);
            }
            EffectCommand::LowPass(low_pass) => {
                process_lowpass_by_channel_chunks(audio, *low_pass, schedule);
            }
...
fn process_treble_by_channel_chunks(
    audio: &mut AudioBuffer,
    treble: Treble,
    schedule: &ChunkSchedule,
) {
...
fn process_equalizer_by_channel_chunks(
    audio: &mut AudioBuffer,
    equalizer: Equalizer,
    schedule: &ChunkSchedule,
) {
...
fn process_lowpass_by_channel_chunks(
    audio: &mut AudioBuffer,
    low_pass: LowPass,
    schedule: &ChunkSchedule,
) {
...
fn process_highpass_by_channel_chunks(
    audio: &mut AudioBuffer,
    high_pass: HighPass,
    schedule: &ChunkSchedule,
) {
