...
/// Crate-local result type for biquad primitive constructors.
pub type BiquadResult<T> = std::result::Result<T, BiquadError>;

/// Errors produced by reusable biquad DSP primitive constructors and designs.
...
///
/// ```
/// use auralis_dsp::{Biquad, BiquadCoefficients};
///
...
impl BiquadCoefficients {
    ...
    /// # Errors
    ///
    /// Returns [`BiquadError::InvalidCoefficients`] if any coefficient is
    /// NaN or infinite.
    pub fn normalized(b0: f64, b1: f64, b2: f64, a1: f64, a2: f64) -> BiquadResult<Self> {
        let coefficients = Self { b0, b1, b2, a1, a2 };
        if coefficients.all_finite() {
            Ok(coefficients)
        } else {
            Err(BiquadError::InvalidCoefficients)
        }
    }
    ...
    pub fn from_raw(b0: f64, b1: f64, b2: f64, a0: f64, a1: f64, a2: f64) -> BiquadResult<Self> {
        if ![b0, b1, b2, a0, a1, a2]
            .iter()
            .all(|value| value.is_finite())
            || a0.to_bits() == 0.0_f64.to_bits()
        {
            return Err(BiquadError::InvalidCoefficients);
        }
...
#[cfg(test)]
mod tests {
    use super::{Biquad, BiquadCoefficients, BiquadError, BiquadState};
    use auralis_core::{
    ...
    fn invalid_coefficients_are_rejected() {
        assert_eq!(
            ...
            BiquadError::InvalidCoefficients
        );
        assert_eq!(
