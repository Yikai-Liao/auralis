...
use crate::{BiquadCoefficients, BiquadError, BiquadResult};
...
impl BiquadWidth {
    ...
    fn alpha(self, frequency_hz: f64, w0: f64, sin_w0: f64) -> BiquadResult<f64> {
        let alpha = match self {
            ...
            Self::Slope(_) => Err(BiquadError::InvalidDesign),
        }?;
        ...
        match self {
            Self::Slope(slope) => {
                if !slope.is_finite() || slope <= 0.0 || slope > 1.0 {
                    return Err(BiquadError::InvalidDesign);
                }
...
impl BiquadCoefficients {
    ...
    /// # Errors
    ///
    /// Returns [`BiquadError::InvalidDesign`] if the sample rate,
    ...
    /// # Errors
    ///
    /// Returns [`BiquadError::InvalidDesign`] if a design parameter is
...
impl RbjDesign {
    fn new(sample_rate_hz: f64, frequency_hz: f64) -> BiquadResult<Self> {
...
fn positive(value: f64) -> BiquadResult<f64> {
    if value.is_finite() && value > 0.0 {
        Ok(value)
    } else {
        Err(BiquadError::InvalidDesign)
    }
}

fn db_to_rbj_amplitude(gain_db: f64) -> BiquadResult<f64> {
...
fn finite_design_value(value: f64) -> BiquadResult<f64> {
    if value.is_finite() {
        Ok(value)
    } else {
        Err(BiquadError::InvalidDesign)
...
#[cfg(test)]
mod tests {
    use super::{BiquadCoefficients, BiquadError, BiquadWidth};
