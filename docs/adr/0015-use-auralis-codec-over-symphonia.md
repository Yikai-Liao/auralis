...
The default decode backend is Symphonia. WAV decode also tries Symphonia first.
`hound` is allowed only as a WAV fallback or special-case WAV path when
Symphonia cannot handle an Auralis-supported case. `hound` is not the primary
decode architecture.
...
Encoding support is format-specific and must be planned explicitly. Symphonia
decode support does not imply that Auralis should write a local encoder for the
same format.
