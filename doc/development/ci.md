...
| `golden-sox-ng` | planned | pull request, push, and release/gnhf gate | required SoX-ng golden checks with missing-oracle failure in release contexts | `ci/golden-sox-ng.md` |
| `external-binary-release` | planned | manual release in dependency repository | publish third-party benchmark/test binaries such as `sox_ng` | `ci/external-binary-release.md` |
| `benchmark-effects` | planned | scheduled and manual | stable effect benchmark subset using fresh output directories | `ci/benchmark-effects.md` |
| `upload-bencher` | planned | scheduled benchmark completion | upload benchmark JSON to Bencher Cloud | `ci/upload-bencher.md` |

Create new CI job documents from
