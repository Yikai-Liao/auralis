...
Feature 8.1.1 and Feature 8.1.2 are now complete: generic WAV decode/high-level
open now cover PCM8, PCM16, and PCM24, and WAV writes can now target PCM8,
PCM16, or PCM24 through `WavEncodeOptions` while the legacy `write_wav` path
stays PCM16-only for backward compatibility. The next unchecked 8.x leaf is
WAV PCM32.
